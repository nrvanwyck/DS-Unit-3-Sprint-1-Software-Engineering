...
It is not enough for code to be functional; it should also be readable.  You want code to be easy to follow so it is easy to debug.  Annotations are helpful here, as are good variable names.  Explicit is better than implicit.  Code review can also be helpful for spotting redundancies and finding simpler ways of doing things, though this shouldn't come at the expense of readability.  Others may also be able to identify possible problem cases that the code fails to handle, and can suggest ways of handling those cases.

- We have an awful lot of computers here, and it gets pretty confusing with
  slightly different things running on all of them. How could containers help us
  improve this situation?

Containerized applications include all the dependencies you need to run an application.  This allows you to abstract the application from a particular local environment.  A containerized application will run the same for anyone with the container.  You don't need to worry about differences between local environments; in fact, you don't even need to know what dependencies are required to run the application in order to run it.  Once you download the container from the internet, you are good to go.  This also makes it easier to collaborate.
